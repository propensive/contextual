...
object scalac {
  ...
  case class TypecheckError(message: String) extends Compilation
  case class Returns(returnType: Fqt) extends Compilation
        ...
        case lit@Literal(index, string) =>
          ...
          try {
            ...
            q"""_root_.contextual.data.scalac.Returns(
                _root_.contextual.data.fqt.Fqt($returnType))"""
          } catch {
            case e: TypecheckException =>
              q"""_root_.contextual.data.scalac.TypecheckError(${e.msg})"""
          }
        case hole: Hole =>
